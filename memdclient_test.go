...
func TestResolveRequestCompressedEmptyValue(t *testing.T) {
	...
	if !result.called {
		t.Fatal("callback was not invoked — the event was dropped due to snappy decompression failure on empty value")
	}
	if result.err != nil {
		t.Fatalf("expected no error in callback, got: %v", result.err)
	}
	// The compressed flag should have been cleared.
	if result.datatype&uint8(memd.DatatypeFlagCompressed) != 0 {
		t.Errorf("expected compressed flag to be cleared, got datatype: 0x%x", result.datatype)
	}
}
...
func TestResolveRequestCompressedNonEmptyValue(t *testing.T) {
	client := newTestMemdClient(false)
	...
	if string(result.value) != string(originalValue) {
		t.Errorf("expected decompressed value %q, got %q", originalValue, result.value)
	}
	if result.datatype&uint8(memd.DatatypeFlagCompressed) != 0 {
		t.Errorf("expected compressed flag to be cleared, got datatype: 0x%x", result.datatype)
	}
...
func TestResolveRequestDecompressionDisabled(t *testing.T) {
	...
	if result.datatype&uint8(memd.DatatypeFlagCompressed) == 0 {
		t.Errorf("expected compressed flag to be preserved when decompression is disabled, got datatype: 0x%x", result.datatype)
	}
}
